...
(function (global, factory) {
  ...
  function Rolldate(config) {
      ...
      if (config.value) {
          ...
          console.log(config.value.replace(/-/g, '/').replace(/[^\d/:\s]/g, ''));
          // 默认解决报错
          ...
          switch (config.format) {
              case "hh:mm":
                  curVal = curYear + '-' + curMonth + '-' + curDay + ' ' + config.value;
                  break;
              case "hh:mm:ss":
                  curVal = curYear + '-' + curMonth + '-' + curDay + ' ' + config.value;
              default:
          ...
          var str = curVal.replace(/-/g, '/').replace(/[^\d/:\s]/g, ''),
              date = new Date(str);
  ...
  Rolldate.prototype = {
      ...
      createUI: function createUI() {
          ...
          var _loop = function _loop(_i) {
              ...
              if (FormatArr[_i] == 'YYYY') {
                  that.wheelTo(index);
              } else if (FormatArr[_i] == 'am') {
                  that.wheelTo(index);
              } else {
              ...
              // 循环滚动所做的操作
              if (f != 'am') {
                  var endDomLocation = 0;
                  var startDomLocation = _this.$('#' + $id + '>ul li:nth-child(4)').offsetTop;
                  ...
                  if (f == 'MM') {
                      ...
                      endDomLocation = _this.$('#' + $id + '>ul li:nth-child(63)').offsetTop;
                  }                that.on('scroll', function (location) {
                      if (f != 'YYYY') {
              ...
              that.on('scrollEnd', function () {
                  ...
                  if ([domId['YYYY'], domId['MM']].indexOf(that.wrapper.id) != -1 && _this.scroll['DD']) {
                      var prevDay = _this.getscrollDay(_this.scroll['DD']),
                          _day = _this.bissextile(_this.getscrollDay(_this.scroll['YYYY']), _this.getscrollDay(_this.scroll['MM'])),
                          li = '',
                          _preDay = '',
                          _middleDay = '',
                          _afterDay = '';
                      ...
                      if (_day != _this.$('#' + domId['DD'] + ' li', 1).length) {
                          ...
                          for (var _l = 1; _l <= _day; _l++) {
                              if (_l > _day - 3 && _l < _day + 1) {
                                  _preDay += '<li class="wheel-item">' + (_l < 10 ? '0' + _l : _l) + lang.day + '</li>';
                              }                            _middleDay += '<li class="wheel-item">' + (_l < 10 ? '0' + _l : _l) + lang.day + '</li>';
                              if (_l < 4) {
                                  _afterDay += '<li class="wheel-item">' + (_l < 10 ? '0' + _l : _l) + lang.day + '</li>';
                              }
                          }
                          li = _preDay + _middleDay + _afterDay;
      ...
      event: function event() {
          ...
          confirm.addEventListener('click', function () {
              ...
              for (var f in _this.scroll) {
                  var d = _this.getscrollDay(_this.scroll[f]);
      ...
      getscrollDay: function getscrollDay(scroll) {
          var _this = this;
